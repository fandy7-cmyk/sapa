...
export const handler = async (event) => {
  ...
  try {
    ...
    const [{ klik_hari_ini }] = await sql`SELECT COUNT(*)::INT AS klik_hari_ini FROM klik_log WHERE clicked_at >= CURRENT_DATE`;
    ...
    const klik_7hari = await sql`
      SELECT DATE(clicked_at)::TEXT AS tanggal, COUNT(*)::INT AS jumlah
      FROM klik_log WHERE clicked_at >= NOW() - INTERVAL '7 days'
      GROUP BY DATE(clicked_at) ORDER BY tanggal ASC
    `;
